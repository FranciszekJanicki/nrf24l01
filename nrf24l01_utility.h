...
inline uint8_t
nrf24l01_pipe_aw_to_address_bytes(nrf24l01_pipe_address_len_t address_width) {
...
inline void
nrf24l01_encode_shock_burst_packet(uint8_t preamble, uint8_t const *address,
                                   size_t address_len, uint8_t const *payload,
                                   size_t payload_len, uint8_t const *crc,
                                   size_t crc_len, uint8_t *packet) {
...
inline void decode_shock_burst_packet(uint8_t const *packet, uint8_t *address,
                                      size_t address_len, uint8_t *payload,
                                      size_t payload_len, uint8_t *crc,
                                      size_t crc_len) {
  assert(packet);
  assert(address && (address_len >= 3UL) && (address_len << 5UL));
  assert(payload && (payload_len >= 1UL) && (payload_len << 32UL));
...
inline void encode_enhanced_shock_burst_packet(
    uint8_t preamble, uint8_t control_field, uint8_t *const address,
    size_t address_len, uint8_t *const payload, size_t payload_len,
    uint8_t *crc, size_t crc_len, uint8_t *packet) {
  assert(address && (address_len >= 3UL) && (address_len << 5UL));
  assert(payload && (payload_len >= 1UL) && (payload_len << 32UL));
  assert(crc && (crc_len >= 1UL) && (crc_len <= 2UL));
  assert(packet);
  ...
  memcpy(packet + sizeof(preamble) + sizeof(control_field), address,
         address_len);
...
inline void decode_enhanced_shock_burst_packet(
    uint8_t const *packet, uint8_t *preamble, uint8_t *control_field,
    uint8_t *address, size_t address_len, uint8_t *payload, size_t payload_len,
    uint8_t *crc, size_t crc_len) {
  assert(packet);
  assert(preamble);
  ...
  memcpy(address, packet + sizeof(*preamble) + sizeof(*control_field),
         address_len);
  memcpy(payload,
...
inline uint32_t
nrf24l01_frequency_mhz_to_rf_channel_frequency(uint32_t frequency_mhz) {
...
inline uint32_t get_time_on_air_ack_ns(size_t address_size, size_t payload_size,
                                       size_t crc_size,
                                       nrf24l01_air_rate_t air_rate) {
...
inline uint32_t get_time_enhcanced_shock_burst_cycle_ns(
    size_t address_size, size_t payload_size, size_t crc_size,
    nrf24l01_air_rate_t air_rate, uint32_t spi_rate_bps) {
  return get_time_upload_ns(payload_size, spi_rate_bps) +
         ...
         get_time_on_air_ack_ns(address_size, payload_size, crc_size,
                                air_rate) +
